...
- GET `/api/profile?jwt=...`
  ...
  - Many optional query parameters for customization (e.g., `bg_color`, `text_color`, `show_followers`, `gradient_bg`, `border_radius`). Refer to the API documentation or source code for full details.
  - Content-Type: `image/svg+xml`
  - Cache-Control: `no-cache, no-store, must-revalidate`
