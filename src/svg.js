...
export function renderRecentTracksSVG(items) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const artist = sanitizeText((t.track?.artists || t.artists || []).map((a) => a.name).join(', '), 60);
    const playedAt = t.played_at ? new Date(t.played_at).toLocaleString('fr-FR') : '';
    const y = padding + lineHeight * (i + 2);
    lines += `<text x="${padding}" y="${y}" fill="#B3B3B3" font-size="14" font-family="system-ui">${i + 1}. ${name} — ${artist}${playedAt ? ' • ' + playedAt : ''}</text>\n`;
  }

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Spotify Recently Played">
  <title>Spotify Recently Played</title>
  <rect x="0" y="0" width="${width}" height="${height}" fill="#121212" rx="8" />
  <text x="${padding}" y="${padding + 16}" fill="#FFFFFF" font-size="18" font-family="system-ui" font-weight="600">Titres récemment écoutés</text>
  ${lines}
</svg>`;
}
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Spotify Profile">
  ...
  <text x="16" y="78" fill="#B3B3B3" font-size="12" font-family="system-ui">ID: ${sanitizeText(profile?.id || '', 40)}</text>
</svg>`;
}
